...
# dynamically with any ConfigName, Model or UserId value — no need to hardcode
# dimension values in Terraform.
#
...
#  Row 4 │ Top users by cost           │ Query volume by user             │
...
resource "aws_cloudwatch_dashboard" "main" {
  ...
  dashboard_body = jsonencode({
    widgets = [
      ...
      # ── Row 4, Col 1 ─────────────────────────────────────────────────────────
      # Top users by accumulated cost.
      # Uses Metrics Insights with ORDER BY + LIMIT so the chart stays readable
      # regardless of how many distinct user IDs exist — no static dimension lists.
      {
        type   = "metric"
        x      = 0
        y      = 18
        ...
        height = 6
        properties = {
          title  = "Top 10 Users by Cost (accumulated)"
          view   = "bar"
          region = var.aws_region
          metrics = [[{
            expression = "SELECT SUM(CostPerQuery) FROM \"cloudRAG/Costs\" GROUP BY UserId ORDER BY SUM() DESC LIMIT 10"
            id         = "q1"
            label      = ""
            period     = 86400
          }]]
      ...
      # ── Row 4, Col 2 ─────────────────────────────────────────────────────────
      {
        ...
        properties = {
          ...
          region = var.aws_region
          metrics = [[{
            expression = "SELECT SUM(QueryCount) FROM \"cloudRAG/Costs\" GROUP BY UserId ORDER BY SUM() DESC LIMIT 10"
            id         = "q1"
            label      = ""
            period     = 86400
          }]]
